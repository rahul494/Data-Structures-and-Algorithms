# AlgoExpert - Nth Fibonacci

# The Fibonacci sequence is defined as follows: the first number of the sequence
# is 0, the second number is 1, , and the nth number is the sum of the (n - 1)th
# and (n - 2)th numbers. Write a function that takes in an integer n  and returns the nth Fibonacci number.

# Important note: the Fibonacci sequence is often defined with its first two
# numbers as F0 = 0 and F1 = 1. . For the purpose of this question, the first Fibonacci number is 
# F0; therefore, getNthFib(1) is equal to F0, getNthFib(2) is equal to F1, etc...

# Sample Input #1
#    n = 2

# Sample Output #1
#    1

# Sample Input #2
#    n = 6

# Sample Output #2
#   5

def getNthFibRec(n):
    if(n == 1):
        return 0
    
    elif(n == 2):
        return 1

    else:
        return getNthFibRec(n-1) + getNthFibRec(n-2)

def getNthFibIter(n):
    lastTwo = [0,1]
    
    while(n >= 3):
        n = n - 1
        temp = lastTwo[0]
        lastTwo[0] =  lastTwo[1]
        lastTwo[1] = lastTwo[1] + temp

    return lastTwo[1] if n > 1 else lastTwo[0]

assert getNthFibRec(1) == 0
assert getNthFibIter(1) == 0
assert getNthFibRec(2) == 1
assert getNthFibIter(2) == 1
assert getNthFibRec(5) == 3
assert getNthFibIter(5) == 3
assert getNthFibRec(6) == 5
assert getNthFibIter(6) == 5

print('All tests have passed sucessfully')
